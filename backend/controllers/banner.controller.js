...
exports.createBanner = async (req, res) => {
  try {
    const { title, subtitle, description, image, mobileImage, buttonText, buttonLink, displayOrder, startDate, endDate } = req.body;
    const id = await database.insert(
      `INSERT INTO hero_banners (title, subtitle, description, image, mobile_image, button_text, button_link, display_order, start_date, end_date)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [title, subtitle, description, image, mobileImage, buttonText, buttonLink, displayOrder || 0, startDate, endDate]
    );
